...
<!--
  CONDITIONAL SECTION: Include this section for all features EXCEPT:
  - Trivial configuration changes
  - Documentation-only changes

  For most features, include at least Unit Testing subsection.
  Add Integration/E2E subsections based on complexity.
-->
...
|-------|------------|-------------------|-----------|-----------------|
| P1 | Critical paths | Happy path | Smoke test | 70% |
| P2 | All business logic | Error scenarios | Key user journeys | 80% |
| P3 | Edge cases | All integrations | Full regression | 85%+ |

### Unit Tests

**Scope**: [Business logic, utilities, pure functions, domain models]

**Framework**: [e.g., JUnit 5, pytest, Jest, Vitest]

**Mocking Strategy**:
- **Mock**: [External APIs, database, file system, time]
- **Don't mock**: [Domain models, value objects, pure functions]

**Critical Test Cases**:
- [Test case 1]: [What it validates - e.g., "User creation with valid data succeeds"]
- [Test case 2]: [What it validates - e.g., "User creation with duplicate email fails"]
...
<!--
  DELETE this subsection if feature has no external dependencies or database interactions
-->

**Scope**: [Component interactions, database queries, API contracts, message queue operations]

**Framework**: [e.g., Testcontainers, pytest-docker, Supertest]

**Test Environment**:
- **Database**: [Testcontainers with real database / In-memory database / Docker Compose]
- **External services**: [Mocked via WireMock/MockServer / Test environment]
- **Message queue**: [Testcontainers / In-memory implementation]

**Critical Test Cases**:
- [Test case 1]: [e.g., "User data persisted to database correctly"]
- [Test case 2]: [e.g., "Transaction rollback on failure"]
- [Test case 3]: [e.g., "Event published to message queue"]

### End-to-End Tests

<!--
  DELETE this subsection if feature is backend-only with no user-facing flows
-->

**Scope**: [Complete user workflows, critical business processes]

**Framework**: [e.g., Playwright, Cypress, Selenium, REST Assured]

**Test Environment**: [Staging environment / Dedicated test environment / Production-like]
...
<!--
  DELETE this subsection if no service-to-service communication or external API contracts
-->

**Tool**: [Pact / Spring Cloud Contract / OpenAPI validation]

**Provider-Consumer Pairs**:
- **[Consumer service]** → **[Provider service]**: [Contract description]

### Security Tests

<!--
  DELETE this subsection if feature has no security concerns beyond standard practices
-->

**SAST (Static Analysis)**:
- **Tool**: [SonarQube / Snyk Code / Semgrep / CodeQL]
- **Check for**: [SQL injection, XSS, hardcoded secrets, insecure crypto]

**Dependency Scanning**:
- **Tool**: [Snyk / Dependabot / npm audit / pip-audit]
- **Frequency**: [On every PR / Daily / Weekly]

**Secrets Scanning**:
- **Tool**: [TruffleHog / git-secrets / Gitleaks]
- **Scope**: [Pre-commit hook / CI pipeline]
...
<!--
  ...
  - Feature requires gradual rollout (not all-or-nothing)
...
**Tool**: [LaunchDarkly / Unleash / Custom / Environment variable]

**Rollout Plan**:
1. **Deploy with flag OFF** (dark launch) - [Date/Phase]
2. **Enable for internal users** (dogfooding) - [Date/Phase]
3. **Enable for 1% of users** (canary) - [Date/Phase]
4. **Enable for 10% of users** (early rollout) - [Date/Phase]
5. **Enable for 50% of users** (A/B test if needed) - [Date/Phase]
6. **Enable for 100% of users** (full rollout) - [Date/Phase]

**Monitoring during rollout**:
- **Metrics to watch**: [Error rate, latency, business metrics]
- **Rollback trigger**: [Error rate > 5%, latency > 2x baseline, user complaints]

**Kill Switch**: [How to quickly disable - feature flag toggle, emergency deploy, circuit breaker]
