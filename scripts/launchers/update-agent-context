#!/usr/bin/env bash
# Auto-generated OS-agnostic launcher for update-agent-context
...
if [[ "$OS_TYPE" == "windows" ]]; then
  ...
  if command -v pwsh &> /dev/null; then
    ...
    exec powershell -File "$SCRIPT_DIR/../powershell/update-agent-context.ps1" "$@"
  else
    # Fallback to bash if PowerShell not available (e.g., Git Bash on Windows)
    exec bash "$SCRIPT_DIR/../bash/update-agent-context.sh" "$@"
  fi
else
