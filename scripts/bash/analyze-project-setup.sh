...
# Output results
if $JSON_MODE; then
    printf '{"PROJECT_PATH":"%s","PROJECT_NAME":"%s","ANALYSIS_DIR":"%s","ANALYSIS_REPORT":"%s","UPGRADE_PLAN":"%s","RECOMMENDED_CONSTITUTION":"%s","RECOMMENDED_SPEC":"%s","DEPENDENCY_AUDIT":"%s","METRICS_SUMMARY":"%s","DECISION_MATRIX":"%s","ANALYSIS_DEPTH":"%s","FOCUS_AREAS":"%s","TARGET_HAS_GIT":"%s","TIMESTAMP":"%s"}\n' \
        "$PROJECT_PATH" "$PROJECT_NAME" "$ANALYSIS_DIR" "$ANALYSIS_REPORT" "$UPGRADE_PLAN" "$RECOMMENDED_CONSTITUTION" "$RECOMMENDED_SPEC" "$DEPENDENCY_AUDIT" "$METRICS_SUMMARY" "$DECISION_MATRIX" "$ANALYSIS_DEPTH" "$FOCUS_AREAS" "$TARGET_HAS_GIT" "$TIMESTAMP"
else
