...
if [[ ! -d "$FEATURE_DIR" ]]; then
    ...
    echo "Run /speckitsmart.specify first to create the feature structure." >&2
    exit 1
fi

if [[ ! -f "$IMPL_PLAN" ]]; then
    echo "ERROR: plan.md not found in $FEATURE_DIR" >&2
    echo "Run /speckitsmart.plan first to create the implementation plan." >&2
    exit 1
fi
...
if $REQUIRE_TASKS && [[ ! -f "$TASKS" ]]; then
    ...
    echo "Run /speckitsmart.tasks first to create the task list." >&2
    exit 1
fi
